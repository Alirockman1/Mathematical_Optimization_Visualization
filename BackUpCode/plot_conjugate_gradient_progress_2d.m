function plot_conjugate_gradient_progress_2d(optimizationData, objectiveFunction, varargin)
    parser = inputParser;
...
%    parser.addParameter('VideoWriterOptions', {});
    ...
    title(sprintf('%s optimization process', options.OptimizationMethod), ...
    ...
    if isequal(optimizationData.ProblemData.Options.LineSearchFunction, @line_search_backtracking)
        ...
        for i = 1:nIter
            ...
            stepSize_sweep = linspace(0, 1, 100); % Sweeping from 0 to 1
            ...
            xlabel('Step size ($\alpha$)', 'Interpreter', 'latex', 'FontSize', 12);
            ylabel('Objective function value',  'Interpreter', 'latex', 'FontSize', 12);
            title(['Iteration ', num2str(i)], 'FontName', 'Arial', 'FontSize', 12);
            set(gca, 'FontName', 'Arial', 'FontSize', 12);  % For tick labels
            grid on;
