function plot_modified_newton_progress_2d(optimizationData, objectiveFunction, varargin)
    parser = inputParser;
    ...
    parser.addParameter('LineSearchFunction', @line_search_backtracking, @(x) isa(x, 'function_handle'));
...
%    parser.addParameter('VideoWriterOptions', {});
    ...
    title('Modified newton optimization process');
    xlabel('x1');
    ylabel('x2');
    if length(optimizationData.InitialData.OptimumCandidate) == 3
        zlabel('x3');
    ...
    if isequal(options.LineSearchFunction, @line_search_backtracking)
        ...
        for i = 1:nIter
            ...
            stepsize_sweep = linspace(0, 2, 100);
            ...
            % Clear figure and plot new iteration (before plotting any points)
            clf;
            hold on;
            plot(stepsize_sweep, objective, 'b-', 'LineWidth', 2);  % Plot the sweep curve
            ...
            % Save the final video/GIF frame
            if (options.CreateVideoGifIterations)
                currentFrame = getframe(gcf);
                writeVideo(videoHandle, currentFrame);
    ...
    elseif isequal(options.LineSearchFunction, @line_search_golden_ratio)
        ...
        for i = 1:nIter
            ...
            % Compute objective function values for the curve
            for j = 1:length(stepsize_sweep)
                objective(j) = evaluate_optimization_objective(objectiveFunction, currentDesign + stepsize_sweep(j) * searchDirection);
            end
    
            % Get step sizes used in the iteration
            stepsize_history = optimizationData.IterationData(i).StepSizeData;
            final_stepsize = stepsize_history(end);
    
            % Clear figure, plot sweep curve first
            clf;
            hold on;
            plot(stepsize_sweep, objective, 'b-', 'LineWidth', 2); % Plot the objective curve
            xlabel('Step size (\alpha)');
            ylabel('Objective function value');
            title(['Iteration ', num2str(i)]);
            grid on;
            ...
            % Plot step-size intersections with different colors
            for j = 1:length(stepsize_history)
                % Evaluate objective at the step size
                objectiveVariable = evaluate_optimization_objective(objectiveFunction, currentDesign + stepsize_history(j) * searchDirection);
    
                % Choose marker style
                if j < length(stepsize_history)
                    if j <= 2
                        markerColor = [0.5, 0.5, 0.5]; % Grey for initial bracketing
                        markerSize = 6;
                    else
                    ...
                    markerColor = 'b'; % Blue for final minimum
                    markerSize = 10;
                end
    
                % Plot the point on the existing curve
                plot(stepsize_history(j), objectiveVariable, 'o', ...
                    'MarkerSize', markerSize, 'MarkerEdgeColor', 'k', ...
                    'MarkerFaceColor', markerColor, 'LineWidth', 1.5);
    
                % Add vertical line at the step size
                xline(stepsize_history(j), '--k', ['\alpha_', num2str(j)], ...
                ...
                pause(frameDelay); % For animation effect
            end
    
            % Plot final step size (alpha*) as red dotted line
            xline(final_stepsize, '--r', '\alpha_*', 'LineWidth', 2, ...
                  'LabelVerticalAlignment', 'middle', 'LabelHorizontalAlignment', 'left', 'FontSize', 12);
            ...
            pause(frameDelay); % Before next iteration
        end
    end
    ...
    xlabel('x1');
    ylabel('x2');
    title('Modified newton optimization progress');
