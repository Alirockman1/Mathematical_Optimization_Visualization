function [optimumCandidate, optimumObjectiveValue, optimizationData] = optimization_quasi_newton(objectiveFunction, initialDesign, designSpaceLowerBound, designSpaceUpperBound, varargin)
%OPTIMIZATION_QUASI_NEWTON Quasi-Newton optimization with BFGS Hessian approximation.
%
%   [OPTIMUMCANDIDATE, OPTIMUMOBJECTIVEVALUE, OPTIMIZATIONDATA] = ...
%       OPTIMIZATION_QUASI_NEWTON(OBJECTIVEFUNCTION, INITIALDESIGN, ...
%       DESIGNSPACELOWERBOUND, DESIGNSPACEUPPERBOUND, ...) implements a
%       quasi-Newton optimization method using BFGS Hessian approximation
...
%       'HessianOptions'      - Options for Hessian approximation
%
%   OUTPUTS:
...
%
%   FUNCTIONALITY:
%       1. Implements BFGS quasi-Newton method
%       2. Uses line search to ensure convergence
%       3. Maintains positive definite Hessian approximation
%       4. Handles bound constraints
%       5. Provides detailed iteration logging
%
%   NOTES:
%       - Uses identity matrix as initial Hessian approximation
%       - Automatically enforces design space bounds
%       - Detailed optimization data only computed if requested (nargout=3)
%       - More memory efficient than full Newton's method
%
%   REFERENCES:
%       [1] Nocedal, J., Wright, S.J. (2006). "Numerical Optimization"
%
%   SEE ALSO:
%       optimization_newton, optimization_conjugate_gradient, quasi_newton_bfgs
%
...
%   limitations under the License.
   ...
   % Parse input arguments
    ...
    iteration = 1;
    hasConverged = false;
    inverseQuasiHessianPrevious = eye(length(currentDesign));
    gradientPrevious = [];
    previousDesign = currentDesign;

    while ~hasConverged && iteration <= options.MaxIterations
        % compute gradient and approximate inverse Hessian
        ...
        [inverseQuasiHessian, ~] = quasi_newton_bfgs(currentDesign, gradient, previousDesign, gradientPrevious, inverseQuasiHessianPrevious);

        % Compute search direction (Newton step)
        searchDirection = -(inverseQuasiHessian * gradient(:))';
        ...
        optimizationData.IterationData(iteration) = struct(...
            ...
            'InverseQuasiHessian', inverseQuasiHessian, ...
            'StepSize', stepSize, ...
        ...
        inverseQuasiHessianPrevious = inverseQuasiHessian;
