...
inequalityConstraintFunction = @(x) [ x(:,1) + x(:,2) - 2; -x(:,1) + 2*x(:,2) - 2; 2* x(:,1) + x(:,2) - 3];
equalityConstraintFunction = [];
initialDesign = [0.1, 0.1];
designSpaceLowerBound = [0,0];
designSpaceUpperBound = [1.5, 1.5];

[optimumCandidate, optimumObjectiveValue, optimizationData] = optimization_active_set(...
...
plot_constraint_optimization_progress_slider(optimizationData, objectiveFunction, 'OptimizationMethod', 'Quadratic programming - active set', 'SaveFolder',saveFolder);
